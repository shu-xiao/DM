...
def prepare(dataset):
    os.system('rm -rf step3_inputdataset.txt')
    f = open(dataset,'r')
    for line in f:
        print line
        a=[]
        ...
        datasetdetail=[a,b]
        print datasetdetail
        name='crab status -d crab_projects_MonoHStep2/crab_step2-'+datasetdetail[1]+' | grep -a "Output dataset:" | awk -v my_var='+datasetdetail[1]+' \'{print $3,my_var}\'>> step3_inputdataset.txt'
        print name
        os.system(name)
...
def submit(official):
    ...
    for line in f:
        ...
        if not official:
            ...
            name='crab submit -c crabConfig_step3.py General.requestName=step3-'+datasetdetail[1]+' Data.inputDataset='+datasetdetail[0]+' Data.inputDBS=global'
        print name
        os.system(name)
...
if sys.argv[1] == "prepare" :
    if len(sys.argv) != 3 :
        print "Usuage: python MultiCrab_step3.py prepare step2_inputdataset.txt"
        exit (1)
    else:
